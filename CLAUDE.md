...
5. `ConversationController`/`ConversationView` — 모드 분기가 UX의 축:
   - **삽입 모드** (선택 텍스트 없음): 패널엔 입력창 하나만. 응답은 ThinkTagFilter 통과 후
     ~100ms 버퍼로 대상 텍스트박스에 실시간 타이핑. applyMode 설정 무시.
     **전송 즉시 패널을 숨긴 뒤 타이핑** — 합성 키 이벤트는 key window로 라우팅되므로
     패널이 key인 채로 typeText 하면 패널이 이벤트를 먹는다 (상세: progress/10). 취소는 핫키 재입력.
   - **선택 모드** (선택 텍스트 있음): 미리보기+transcript+교체 버튼, multi-turn.
     applyMode(preview/immediate)는 여기만 적용.
