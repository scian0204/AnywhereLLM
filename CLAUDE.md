...
5. `ConversationController`/`ConversationView` — UX 분기 = 선택 유무 × applyMode:
   - **Transcript UX** (선택 있음, 또는 applyMode=preview): 패널에 스트리밍 표시 +
     multi-turn. preview면 확정 버튼(선택=교체/무선택=삽입), immediate+선택은 완료 시 자동 교체.
   - **실시간 타이핑** (선택 없음 + applyMode=immediate): 응답을 ThinkTagFilter 통과 후
     ~100ms 버퍼로 대상 텍스트박스에 타이핑. **전송 즉시 패널을 숨긴 뒤 타이핑** —
     합성 키 이벤트는 key window로 라우팅되므로 패널이 key인 채로 typeText 하면
     패널이 이벤트를 먹는다 (상세: progress/10). 취소는 핫키 재입력.
